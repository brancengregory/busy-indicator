...
use esp_hal::gpio::{Io, Input, InputConfig, Level, Output, OutputConfig, Pull};
use esp_hal::handler;
use esp_hal::timer::timg::TimerGroup;
use esp_radio::esp_now::{BROADCAST_ADDRESS, PeerInfo, EspNow};
...
async fn main(spawner: Spawner) -> ! {
    ...
    let (mut wifi_controller, interfaces) =
		...
		spawner.spawn(traffic_task(led_local_red, led_local_green, led_remote_red, led_remote_green)).unwrap();
		...
		match select(esp_now.receive_async(), SEND_CHANNEL.receive()).await {
			Either::First(res) => {
				...
				if !data.is_empty() {
					...
					info!("Data received from peer: {}", if peer_red { "RED" } else { "GREEN" });
			...
			Either::Second(local_red_state) => {
				...
				info!("Broadcasted local state: {}", if local_red_state { "RED" } else { "GREEN" });
			}
		}
	}
}
